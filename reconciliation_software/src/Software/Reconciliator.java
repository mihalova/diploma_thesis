...
public class Reconciliator {
        ...
        if (G_rooted == null) {
            for (Edge e : G_unrooted.getEdges()) {
                ...
                for (Four<Double, Double, Double, Double> t : intervals) {
                    RootedIntervalNode u = new RootedIntervalNode(uTree.getRoot());
                    RootedIntervalNode v = new RootedIntervalNode(vTree.getRoot());
                    //
    ...
    List<Four<Double, Double, Double, Double>> getIntervals(Edge e, double step) {
        ...
        double minLengthRight = EPSILON;
        double maxLengthRight = intervalSize;

        //intervaly s krokom
        if (step > 0.0) {
            while (maxLengthRight < totalMaxLength && maxLengthLeft > 0.0) {
                intervals.add(new Four<>(minLengthLeft, maxLengthLeft, minLengthRight, maxLengthRight));
                minLengthLeft -= step;
    ...
    // zaokruhlenie double na 6 desatinnych miest
