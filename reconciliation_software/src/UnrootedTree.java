...
class UnrootedTree {
    ...
    public boolean equalTopology(UnrootedTree other) {
    ...
    public void createFromTrees(ArrayList<UnrootedTree> viableTrees, String method, double methodParam) {
        ArrayList<ArrayList<UnrootedNode>> listsOfNodes = new ArrayList<>();
        for(UnrootedTree tree: viableTrees){
            Collections.sort(tree.nodes, new Comparator<UnrootedNode>(){
                public int compare(UnrootedNode n1,UnrootedNode n2){
                    return n1.getName().compareTo(n2.getName());
                }});
            listsOfNodes.add(tree.nodes);
        }
        for (int i = 0; i < listsOfNodes.get(0).size(); i++) {
            UnrootedNode newNode = new UnrootedNode(listsOfNodes.get(0).get(i).getName());
            ...
            for (int j = 0; j < listsOfNodes.size(); j++) {
                UnrootedNode node = listsOfNodes.get(j).get(i);
                Collections.sort(node.getEdges(), new Comparator<Edge>(){
                ...
                for (int k = 0; k < node.getEdges().size(); k++) {
                    ...
                    listForNode.add(node.getEdges().get(k).getMinL());
                }
            ...
            for (int j = 0; j < lensForEdge.size(); j++) {
                ...
                UnrootedNode otherNode = new UnrootedNode(nodeNames.get(j));
                if(nodes.contains(otherNode)){
                    otherNode = nodes.get(nodes.indexOf(otherNode));
                } else {
                    nodes.add(otherNode);
                }
                Edge e = new Edge(newNode, otherNode, interval[0], interval[1]);
                if(!edges.contains(e)) {
                    edges.add(e);
                    newNode.addEdge(e);
                    otherNode.addEdge(e);
        ...
        Collections.sort(nodes, new Comparator<UnrootedNode>(){
            public int compare(UnrootedNode n1,UnrootedNode n2){
                return n1.getName().compareTo(n2.getName());
            }});
    }
